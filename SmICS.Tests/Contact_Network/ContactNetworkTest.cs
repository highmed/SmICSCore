...
namespace SmICSDataGenerator.Tests.Contact_Network
{
	public class ContactNetworkTest
	{
		...
		private ContactModel CreateExpactedContactModel(List<int> contactOrder)
        {
			...
			{
				List<PatientMovementModel> movementResult = ExpectedResultJsonReader.ReadResults<PatientMovementModel, PatientIDs>(movementPath, movementPath, i, ExpectedType.PATIENT_MOVEMENT);
				List<LabDataModel> labResult = ExpectedResultJsonReader.ReadResults<LabDataModel, PatientIDs>(movementPath, labPath, i, ExpectedType.LAB_DATA);
