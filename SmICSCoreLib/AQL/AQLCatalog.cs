...
namespace SmICSCoreLib.AQL
{
    public sealed class AQLCatalog
    {
        ...
        {
            return new AQLQuery($"SELECT m/data[at0001]/items[at0004]/value/value as Beginn, m/data[at0001]/items[at0005]/value/value as Ende, k/items[at0048]/value/defining_code/code_string as Fachabteilung, k/items[at0046]/value/value as StationID FROM EHR e CONTAINS COMPOSITION c CONTAINS ADMIN_ENTRY m[openEHR-EHR-ADMIN_ENTRY.hospitalization.v0] CONTAINS CLUSTER k[openEHR-EHR-CLUSTER.location.v1] where c/name/value='Patientenaufenthalt' and e/ehr_id/value = '{ parameter.PatientID }' and ((m/data[at0001]/items[at0004]/value/value <= '{ parameter.Starttime.ToString("o") }' and m/data[at0001]/items[at0005]/value/value >= '{ parameter.Starttime.ToString("o") }') or (m/data[at0001]/items[at0004]/value/value > '{ parameter.Starttime.ToString("o") }' and m/data[at0001]/items[at0005]/value/value >= '{ parameter.Starttime.ToString("o") }' and m/data[at0001]/items[at0005]/value/value <= '{ parameter.Endtime.ToString("o") }')) ORDER BY m/data[at0001]/items[at0004]/value/value ASC");
        }
        public static AQLQuery ContactPatients(ContactPatientsParameter parameter)
        {
            return new AQLQuery($"SELECT e/ehr_id/value as PatientID, o/data[at0001]/items[at0004]/value/value as Beginn, o/data[at0001]/items[at0005]/value/value as Ende FROM EHR e CONTAINS COMPOSITION c CONTAINS ADMIN_ENTRY o[openEHR-EHR-ADMIN_ENTRY.hospitalization.v0] CONTAINS CLUSTER l[openEHR-EHR-CLUSTER.location.v1] where c/name/value='Patientenaufenthalt' and ((o/data[at0001]/items[at0004]/value/value >= '{ parameter.Starttime.ToString("o") }' and o/data[at0001]/items[at0004]/value/value <= '{ parameter.Endtime.ToString("o") }') or (o/data[at0001]/items[at0004]/value/value <= '{ parameter.Starttime.ToString("o") }' and o/data[at0001]/items[at0005]/value/value > '{ parameter.Starttime.ToString("o") }')) and ((o/data[at0001]/items[at0004]/value/value <= '{ parameter.SuperStarttime.ToString("o") }' and o/data[at0001]/items[at0005]/value/value >= '{ parameter.SuperStarttime.ToString("o") }') or (o/data[at0001]/items[at0004]/value/value > '{ parameter.SuperStarttime.ToString("o") }' and o/data[at0001]/items[at0004]/value/value <= '{ parameter.SuperEndtime.ToString("o") }' and o/data[at0001]/items[at0005]/value/value >= '{ parameter.SuperStarttime.ToString("o") }')) and l/items[at0048]/value/defining_code/code_string = '{ parameter.Departement }' and l/items[at0046]/value/value = '{ parameter.WardID }' and not e/ehr_id/value = '{ parameter.PatientID }'");
        }
        ...
        /*public static AQLQuery PatientStay(PatientListParameter patientList)
        {
            return new AQLQuery($@"SELECT e/ehr_id/value as PatientID,
                                ...
                                h/items[at0004]/value/value as Beginn,
                                h/items[at0005]/value/value as Ende,
                                h/items[at0006]/value/value as Bewegungsart_l,
                                s/items[at0027]/value/value as StationID,
                                s/items[at0027]/value/value as Raum,
                                ...
                                AND i/name/value = 'Versorgungsfall'
        ...
        {
            return new AQLQuery($"SELECT p/data[at0001]/items[at0071]/value/value as Beginn, b/data[at0001]/items[at0011,'Datum/Uhrzeit der Entlassung']/value/value as Ende FROM EHR e CONTAINS COMPOSITION c[openEHR-EHR-COMPOSITION.fall.v0] CONTAINS (ADMIN_ENTRY p[openEHR-EHR-ADMIN_ENTRY.admission.v0] and ADMIN_ENTRY b[openEHR-EHR-ADMIN_ENTRY.discharge_summary.v0]) WHERE e/ehr_id/value = '{ parameter.PatientID }' and c/context/other_context[at0001]/items[at0003,'Fall-Kennung']/value/value = '{ parameter.CaseID }'");
        }
        ...
        {
            return new AQLQuery($@"SELECT e/ehr_id/value as PatientID,
                                    c/context/start_time/value as Befundzeit,
                                    y/items[at0001]/value/value as FallID,
                                    a/items[at0001]/value/value as LabordatenID,
                                    ...
                                    a/items[at0015]/value/value as Zetpunktprobeneingang,
                                    d/items[at0024]/value/value as Keim_l,
                                    d/items[at0024]/value/defining_code/code_string as KeimID,
                                    d/items[at0001,'Nachweis']/value/value as Nachweis,
                                    d/items[at0001,'Nachweis']/value/defining_code/code_string as NachweisCode,
                                    d/items[at0001,'Quantitatives Ergebnis']/value/value as Viruslast,
