...
namespace SmICSCoreLib.AQL
{
    public sealed class AQLCatalog
    {
        ...
        public static AQLQuery NECPatientLaborData(string PatientID, TimespanParameter timespan)
        {
            ...
            return new AQLQuery("NECPatientLaborData", $"SELECT e/ehr_status/subject/external_ref/id/value as PatientID, z/items[at0015]/value/value as ZeitpunktProbenentnahme, z/items[at0029]/value/defining_code/code_string/value as MaterialID, g/items[at0001]/value/value as Befund, g/items[at0024]/value/defining_code/code_string as KeimID, c/context/start_time/value as Befunddatum FROM EHR e CONTAINS COMPOSITION c[openEHR-EHR-COMPOSITION.report-result.v1] CONTAINS (CLUSTER f[openEHR-EHR-CLUSTER.case_identification.v0] and CLUSTER z[openEHR-EHR-CLUSTER.specimen.v1] and CLUSTER j[openEHR-EHR-CLUSTER.laboratory_test_panel.v0] CONTAINS CLUSTER g[openEHR-EHR-CLUSTER.laboratory_test_analyte.v1]) WHERE c/name/value = 'Virologischer Befund' and g/items[at0001]/name = 'Nachweis' and g/items[at0024]/name = 'Virus' and e/ehr_status/subject/external_ref/id/value = '{PatientID}' and z/items[at0015]/value/value >= '{timespan.Starttime}' and z/items[at0015]/value/value <= '{timespan.Endtime}'");
        }
        ...
        {
            return new AQLQuery("PatientSymptom_VS", $"SELECT e/ehr_status/subject/external_ref/id/value as PatientenID, a/data[at0190]/events[at0191]/data[at0192]/items[at0001]/value/value as NameDesSymptoms, a/data[at0190]/events[at0191]/data[at0192/items[at0151]/value/value as Lokalisation, a/ data[at0190]/events[at0191]/data[at0192]/items[at0152]/value/value as Beginn, a/ data[at0190]/events[at0191]/data[at0192]/items[at0021]/value/value as Schweregrad, a/ data[at0190]/events[at0191]/data[at0192]/items[at0161] as Rueckgang FROM EHR e CONTAINS COMPOSITION c CONTAINS OBSERVATION a[openEHR-EHR-OBSERVATION.symptom_sign.v0] WHERE c/archetype_details/template_id='Symptom' and e/ehr_status/subject/external_ref/id/value matches { patientList.ToAQLMatchString() }");
        }
        public static AQLQuery PatientSymptom_AS(PatientListParameter patientList)
        {
            return new AQLQuery("PatientSymptom_AS", $"SELECT e/ehr_status/subject/external_ref/id/value as PatientenID, a/data[at0001]/items[at0002]/value/value as AusschlussAussage, a/ data[at0001]/items[at0003]/value/value as Diagnose FROM EHR e CONTAINS COMPOSITION c CONTAINS EVALUATION a[openEHR-EHR-EVALUATION.exclusion_specific.v1] WHERE c/archetype_details/template_id='Symptom' and e/ehr_status/subject/external_ref/id/value matches { patientList.ToAQLMatchString() }");
        }

        public static AQLQuery PatientSymptom_US(PatientListParameter patientList)
        {
            return new AQLQuery("PatientSymptom_US", $"SELECT e/ehr_status/subject/external_ref/id/value as PatientenID, a/data[at0001]/items[at0002]/value/value as UnbekanntesSymptom, a/ data[at0001]/items[at0005]/value/value as AussageFehlendeInfo FROM EHR e CONTAINS COMPOSITION c CONTAINS EVALUATION a[openEHR-EHR-EVALUATION.absence.v2] WHERE c/archetype_details/template_id='Symptom' and e/ehr_status/subject/external_ref/id/value matches { patientList.ToAQLMatchString() }");
        }
        //untested
        ...
        public static AQLQuery PatientsOnWard(WardOverviewParameters parameter)
        {
